<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8" import="java.util.ArrayList,dao.*,model.*"%>
<!DOCTYPE html>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>Danh Sách</title>
<style>
.center {
	margin-left: auto;
	margin-right: auto;
}

button {
	
}
</style>

</head>
<%
ArrayList<Khachhang> listKH = new ArrayList<>();
if (request.getParameter("tenkh") != null) {
	KhachhangDAO dao = new KhachhangDAO();
	listKH = dao.getDSKhachhang(request.getParameter("tenkh"));
	session.setAttribute("tenKH", request.getParameter("tenkh"));
}
%>
<body>
	<h2 style="text-align: center;">Quản lý cửa hàng</h2>
	<h2 style="margin-left: 500px; text-align: left">Bảng lương chi
		tiết</h2>
	<h5 style="margin-left: 500px; text-align: left">Thông tin nhân
		Viên</h5>
	<form name="thongTinNhanVien" ,style="align-content: space-between;">
		<h5 style="margin-left: 500px; text-align: left">Name</h5>
		<h5 style="margin-left: 500px; text-align: left">Sinh ngày:</h5>
		<h5 style="margin-left: 500px; text-align: left">Giới tính:</h5>
		<h5 style="margin-left: 500px; text-align: left">Quê Quán:</h5>
		<h5 style="margin-left: 500px; text-align: left">Vị Trí:</h5>
	</form>
	<form name="luongNV">
		<h5 style="margin-left: 500px; text-align: left">Lương Cứng:</h5>
		<h5 style="margin-left: 500px; text-align: left">Lương Hoa Hồng:</h5>
		<table class="center"
			style="width: 50%; border: 1px solid black; border-collapse: collapse;">
			<thead>
				<th style="border: 1px solid black; padding: 0 15px 0 15px;">Id
				
				</td>
				<th style="border: 1px solid black; padding: 0 15px 0 15px;">Ngày
				
				</td>
				<th style="border: 1px solid black; padding: 0 15px 0 15px;">Khách
				
				</td>
				<th style="border: 1px solid black; padding: 0 15px 0 15px;">Tên
					Dịch Vụ
				</td>
				<th style="border: 1px solid black; padding: 0 15px 0 15px;">Đơn
					Giá
				</td>
				<th style="border: 1px solid black; padding: 0 15px 0 15px;">Thành
					Tiền
				</td>
				<th style="border: 1px solid black; padding: 0 15px 0 15px;">%
					hoa hồng
				</td>
				<th style="border: 1px solid black; padding: 0 15px 0 15px;">Thực
					nhận
				</td>
			</thead>
			<%
			if (listKH != null)
				for (int i = 0; i < listKH.size(); i++) {
			%>
			<tr>
				<td style="border: 1px solid black; padding: 0 15px 0 15px;"><%=listKH.get(i).getId()%></td>
				<td style="border: 1px solid black; padding: 0 15px 0 15px;"><%=listKH.get(i).getTen()%></td>
				<td style="border: 1px solid black; padding: 0 15px 0 15px;"><%=listKH.get(i).getSoID()%></td>
				<td style="border: 1px solid black; padding: 0 15px 0 15px;"><%=listKH.get(i).getKieuID()%></td>
				<td style="border: 1px solid black; padding: 0 15px 0 15px;"><%=listKH.get(i).getDienthoai()%></td>
				<td style="border: 1px solid black; padding: 0 15px 0 15px;"><%=listKH.get(i).getEmail()%></td>
				<td style="border: 1px solid black; padding: 0 15px 0 15px;"><%=listKH.get(i).getDiachi()%></td>
				<td style="border: 1px solid black; padding: 0 15px 0 15px;"><button
						onclick="document.location='GDTimtour.jsp'">Chọn</button></td>
			</tr>
			<%
			}
			%>
		</table>
	</form>
	&nbsp;
	<div style ="width:800px;margin: 0 auto;">
    <div style="float: left;"> 
        <form id="thisone" action="post_nrs_users.asp" method="post">
            <input type="submit" name = "" value="Quay Lại" >
        </form>
    </div>
    <div style="float: right;"> 
        <form id="thistoo" action="post_nrs_users.asp" method="post">
            <input type="submit" name = "" value="Lưu và In" >
        </form>
    </div>
</div>
	

</body>
</html>
